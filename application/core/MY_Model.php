...
class MY_Model extends CI_Model {
	...
	{		
		...
		{
			foreach ($input['join'] as $con_join) {
				$this->db->join($con_join[0], $con_join[1], $con_join[2]);
			}
		}
	...
	function get_count($input = array()) {
		if (isset($input['join']) && $input['join'])
		{
			foreach ($input['join'] as $con_join) {
				$this->db->join($con_join[0], $con_join[1], $con_join[2]);
	...
	protected function get_list_set_input($input = array())
	{
		...
		if (isset($input['distinct']) && $input['distinct']) 
		{
			$this->db->distinct($input['distinct']);
		}	
